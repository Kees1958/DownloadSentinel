...
window.SentinelOptions = window.SentinelOptions || (function () {
    ...
    function unlockSections(unlock) {
        ['colorSection', 'domainSection', 'fpSection'].forEach(id => {
            const el = document.getElementById(id);
            if (!el) return;
    ...
    function bindFpSlider() {
        ...
        saveBtn.addEventListener("click", () => {
            const level = FP_LEVELS[parseInt(slider.value, 10)] || "MEDIUM";
            Settings.set({ fpReductionLevel: level }, () => {
                setMsg("fpMsg", "Saved!", "success");
                notifyBackground();
            });
        });
    }
